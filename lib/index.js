...
function scale(name, scale) {
    return getAutoScalingGroup(name)
        .then(autoScaleGroup => scaleAutoScalingGroup(autoScaleGroup.AutoScalingGroupName, scale))
        .then(() => logger.info('Scaled auto scaling group to ' + JSON.stringify(scale)))
        .catch(function(err) {
            logger.error(err);
            process.exit(1);
        });
...
module.exports = function() {
    ...
    program
        ...
        .action((asgname, options) => scale(asgname, {
            min: options.minimum || 0,
            max: options.maximum || 0,
