...
module.exports = (autoScalingGroup, config) => new Promise((resolve, reject) => {
    return getLaunchConfiguration(autoScalingGroup.LaunchConfigurationName)
        .then(validate)
        .then(launchConfig => copyLaunchConfiguration(launchConfig, config))
        ...
        .then(() => logger.info('Update auto scaling group with new ami'));
});
